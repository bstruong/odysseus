...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            #
            # Both builders take _gen_model — the model this turn actually runs —
            # rather than sess.model, so index 0 of the candidate list and index 0
            # of the parallel descriptor list describe the same route by
            # construction. Escalation is deliberately NOT a fallback candidate:
            # the fallback chain is reactive availability failover (pre-content
            # HTTP failures only) and is opt-in/fails-closed, so on a default box
            # it is empty. Escalation is a per-turn override of the PRIMARY route.
            # sess.model stays the separately-tracked requested model.
            ...
            # descriptor[0] describes _gen_model, not sess.model: the builders
            # above are fed _gen_model, so the announced model and the announced
            # endpoint identity refer to the same route. Reporting _gen_model
            # here is what upstream already intends ("the route being used"),
            # not a departure from it — sess.model is carried separately as
            # _requested_model for the requested-vs-actual audit trail.
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                # Seed with the escalation model when it fired, mirroring the
                # agent branch below. llm_core suppresses its model_actual event
                # when the provider reports the model it was asked to run --
                # which is always the case now that candidates[0] IS the
                # escalation model -- so without this seed an escalated chat
                # turn would attribute its answer to the pre-escalation model.
                ...
                # Round 1 runs _gen_model, which is sess.model on an ordinary turn
                # and the escalation model on an escalated one. This map is the
                # per-round ACTUAL model (later rounds are filled from answered_by
                # / model_actual events), so seeding it from _requested_model would
                # mislabel round 1 of an escalated turn.
                _agent_round_models = {1: _gen_model}
                ...
                try:
                    ...
                    async for chunk in stream_agent_loop(
                        ...
                        _gen_model,
                        messages,
                        # What the user actually selected. Kept distinct from
                        # `model` above so an escalated turn reports
                        # requested=sess.model / actual=_gen_model instead of
                        # claiming the escalation model was the one requested.
                        requested_model=_requested_model,
