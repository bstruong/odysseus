...
Generate an image. Line 1 = description, line 2 = model name, line 3 = WxH (e.g. 1024x1024), line 4 = quality.""",
    ...
    "manage_notes": """\
```manage_notes
{"action": "add", "title": "<short label>", "content": "<the full text to save>"}
```
Notes, checklists, AND user reminders. Use this for "create/add/write a note", todos, checklists, and "remind me to X at <time>" — never use memory for note content. For a plain note, ALWAYS include `content` with the actual text the user wants saved — a call with only `title` and no `content` produces a blank, useless note (confirmed live: this is a real, repeatable failure, not hypothetical). For a reminder instead, pair a short `title` (what to do) with a `due_date` (when); `content` is optional there. `due_date` accepts natural language ("tomorrow at 1pm", "in 2 hours", "next monday 9am") or ISO ("2026-05-12T13:00:00"). Actions: `list`, `add` (title, content OR items:[{text,done}], note_type, color, label, due_date), `update`, `delete`, `toggle_item`.""",
...
async def stream_agent_loop(
    ...
    # `model` is the model this run actually executes, which is not necessarily
    # the one the user selected — callers that reroute a single turn (e.g. the
    # zero-grounding escalation in chat_routes) pass the original selection as
    # requested_model. Defaulting to `model` keeps every existing caller's
    # behavior identical.
